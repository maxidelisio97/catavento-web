...
export default function BookingForm({ room }: BookingFormProps) {
  ...
  const labelClass =
    "flex items-center gap-1.5 font-body text-[9px] font-semibold uppercase tracking-[0.2em] text-coral-600/85";
  const iconClass = "shrink-0 text-warm-800/40";
  const valueClass = "mt-1 font-body text-base font-semibold tracking-tight text-warm-900 truncate";
  const valuePlaceholderClass = "mt-1 font-body text-base font-normal text-warm-800/45 truncate";
  const cellButtonClass =
    "w-full h-full text-left px-5 py-3.5 flex flex-col hover:bg-sand-50 transition-colors focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-coral-500";

  return (
    <form onSubmit={handleSubmit} noValidate className="w-full">
      <div className="flex flex-col md:flex-row md:items-center gap-3 w-full">
        {/* Pildora: fechas + hospedes */}
        <div className="flex flex-col md:flex-row md:flex-1 w-full rounded-2xl md:rounded-full border border-stone-300 bg-white divide-y divide-stone-300 md:divide-y-0 md:divide-x">
          {/* Fechas */}
          <div className="relative flex-[1.3] min-w-0">
            ...
            {isPickerOpen && (
              <>
                <div className="fixed inset-0 z-20 bg-warm-900/25 md:hidden" aria-hidden />
                <div
                  ref={datePopoverRef}
                  role="dialog"
                  aria-label="Selecionar datas da estadia"
                  onMouseLeave={() => setHoveredDay(undefined)}
                  className="absolute z-30 left-0 top-full mt-2 w-[340px] max-w-[92vw] rounded-2xl border border-stone-300 bg-sand-50 p-4 shadow-xl shadow-warm-900/20"
                >
                  <DayPicker
                    mode="range"
                    locale={ptBR}
          ...
          <div className="relative flex-1 min-w-0">
            ...
            >
              ...
              <span className="mt-1 flex items-center justify-between gap-2">
                ...
                <LuChevronDown
                  size={14}
                  aria-hidden
                  className={`shrink-0 text-warm-800/40 transition-transform duration-200 ${isGuestsOpen ? "rotate-180" : ""}`}
                />
              </span>
            </button>
            ...
            {isGuestsOpen && (
              <div
                ...
                className="absolute z-30 left-0 top-full mt-2 w-full min-w-[260px] space-y-4 rounded-2xl border border-stone-300 bg-sand-50 p-4 shadow-xl shadow-warm-900/20"
              >
                <GuestCounterRow
                  label="Adultos"
        ...
        <button
          ...
          className="verificar-btn group relative self-center shrink-0 flex items-center justify-center gap-2 w-full md:w-16 h-14 md:h-16 rounded-2xl md:rounded-full bg-coral-600 hover:bg-coral-500 text-sand-50 md:overflow-hidden transition-colors duration-200 active:scale-[0.97] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-coral-400"
        >
